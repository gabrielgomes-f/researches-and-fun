logistic.model.metrics <- function(formula, data.train, data.test, treshold = 0.5){
  ...
  # Calculating AUC
  logistic.auc <- rocit(logistic.pred.probability, data.test$Y)$AUC
  ...
  logistic.output <- data.frame(logistic.performance.metrics, logistic.auc, logistic.chi.square.statistic,
  ...
  names(logistic.output) <- c("logistic_ACC","logistic_TPR","logistic_TNR","logistic_CSI", "logistic_SSI",
                              ...
                              "logistic_F1","logistic_MCC","logistic_AUC","logistic_ChiSquare","logistic_KS",
                              "logistic_RunTime","logistic_ProbMean")
...
randomforest.model.metrics <- function(formula, data.train, data.test, treshold = 0.5){
  ...
  # Calculating AUC
  randomforest.auc <- rocit(randomforest.pred.probability, data.test$Y)$AUC
  ...
  randomforest.output <- data.frame(randomforest.performance.metrics, randomforest.auc, randomforest.chi.square.statistic,
  ...
  names(randomforest.output) <- c("randomforest_ACC","randomforest_TPR","randomforest_TNR","randomforest_CSI", "randomforest_SSI",
                              ...
                              "randomforest_F1","randomforest_MCC", "randomforest_AUC", "randomforest_ChiSquare","randomforest_KS",
                              "randomforest_RunTime","randomforest_ProbMean")
